    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            {confirmCancel ? (
              <div className="flex items-center gap-2">
                <span className="text-[12.5px] text-[#FFA7BB]">Cancel at period end?</span>
      ...
      {status === "cancelled" && (
        <div className="rounded-2xl border border-[#FFB443]/30 bg-[#FFB443]/[0.07] p-5">
          <p className="text-[14px] font-semibold text-[#FFCF87]">Your subscription is cancelled</p>
          ...
          return (
            ...
            >
              ...
              <div className="mt-4 flex items-baseline gap-1">
                ...
                <span className="text-[13px] text-[#5B5B70]">/month</span>
              </div>

              <div className="mt-4 grid grid-cols-3 gap-2 rounded-xl border border-[#1E1E28] bg-black/25 p-2.5 text-center">
                <div>
                  <p className="display text-base">{pkg.totalPosts}</p>
                  <p className="text-[9.5px] uppercase tracking-wider text-[#5B5B70]">posts</p>
                </div>
                <div>
                  <p className="display text-base">{pkg.videosPerMonth || "—"}</p>
                  <p className="text-[9.5px] uppercase tracking-wider text-[#5B5B70]">videos</p>
                </div>
                <div>
                  <p className="display text-base">{pkg.projects}</p>
                  <p className="text-[9.5px] uppercase tracking-wider text-[#5B5B70]">projects</p>
                </div>
              </div>
              ...
              <p className="mt-2.5 text-center text-[11px] text-[#4E4E60]">
      ...
      <p className="rounded-xl border border-[#1E1E28] bg-white/[0.02] px-4 py-3 text-[12px] leading-relaxed text-[#5B5B70]">
        This build processes plan changes locally so you can use the whole product end to end. Wire{" "}
        <code className="text-[#8A8A9E]">/api/billing</code> to Stripe Checkout and fulfil on the
