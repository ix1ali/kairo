  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {CREDIT_PACKS.map((p) => (
            ...
            >
              ...
              <p className="text-[11px] uppercase tracking-wider text-[#5B5B70]">credits</p>
        ...
        {error && (
          <p className="mt-3 rounded-lg border border-[#FF6B8A]/30 bg-[#FF6B8A]/10 px-3 py-2.5 text-[13px] text-[#FFA7BB]">
            {error}
          </p>
        )}
      </div>
          ...
          {CREDIT_ACTIONS.map((a) => (
            <div key={a.key} className="flex items-center justify-between gap-4 px-5 py-3.5">
              <div>
                ...
                <p className="text-[12px] text-[#5B5B70]">{a.description}</p>
              </div>
