    ...
    <div ref={box} className="relative">
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate">
          ...
          <span className="text-[#6C6C80]"> — {current.dialect}</span>
        </span>
        <Icon name="arrowDown" size={14} className={`text-[#6C6C80] transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
      ...
      {open && (
        <div className="absolute z-40 mt-2 max-h-[320px] w-full overflow-hidden rounded-2xl border border-[#22222E] bg-[#0C0C13] shadow-2xl">
          ...
          <div className="max-h-[248px] overflow-y-auto p-1.5">
            {groups.length === 0 && (
              <p className="px-3 py-4 text-center text-[13px] text-[#5B5B70]">No match.</p>
            )}
            {groups.map((g) => (
              <div key={g.language} className="mb-1">
                <p className="px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-widest text-[#4E4E60]">
                  {g.language}
                </p>
                {g.locales.map((l) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block truncate text-[11px] text-[#5B5B70]">{l.country}</span>
                      </span>
                      <span className="shrink-0 text-[11px] text-[#4E4E60]" dir={l.rtl ? "rtl" : "ltr"}>
                        {l.native}
                      </span>
                      {on && <Icon name="check" size={13} className="text-[#A78BFA]" strokeWidth={2.6} />}
                    </button>
                  );
                })}
              </div>
