  ...
  return (
    <div className="rounded-2xl border border-[#1E1E28] bg-[#0C0C13] p-3 shadow-2xl">
      ...
      <div className="flex items-center gap-2 rounded-xl border border-[#7C5CFF]/40 bg-[#7C5CFF]/[0.08] px-3 py-2.5">
        <Icon name="link" size={15} className="text-[#7C5CFF]" />
...
function BrandDnaMock({ t }: { t: Dict["how"] }) {
  return (
    <div className="rounded-2xl border border-[#1E1E28] bg-[#0C0C13] p-4 shadow-2xl">
      <p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-[#7E7E93]">{t.mockBrandRead}</p>
      <div className="mb-3 flex gap-1.5">
        {["#E4732B", "#F2C14E", "#16161F", "#FFF8F0"].map((c) => (
          <span key={c} className="h-8 w-8 rounded-lg border border-[#1E1E28]" style={{ background: c }} />
        ))}
      </div>
  ...
  return (
    <div className="rounded-2xl border border-[#1E1E28] bg-[#0C0C13] p-4 shadow-2xl">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-[10px] font-bold uppercase tracking-widest text-[#7E7E93]">{t.mockYourDays}</p>
        <span className="chip">{t.mockPosts}</span>
      </div>
      <div className="space-y-1.5">
        {weeks.map((tint, wi) => (
          <div key={wi} className="flex items-center gap-2">
            <span className="w-5 text-[9px] font-bold text-[#7E7E93]">W{wi + 1}</span>
      ...
      <div className="flex gap-3">
        <div className="relative h-[92px] w-[74px] shrink-0 overflow-hidden rounded-xl border border-[#22222E] bg-gradient-to-br from-[#7C5CFF]/40 to-[#22D3EE]/20">
          <div className="absolute inset-x-2.5 bottom-3 space-y-1">
            ...
            <span className="block h-1 w-2/3 rounded-full bg-[#22222E]" />
          </div>
        </div>
        <div className="min-w-0 flex-1">
          <div className="mb-1.5 flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-[#7E7E93]">Day 12 · 18:30</span>
          </div>
          ...
          <div className="mt-2.5 flex gap-1.5">
            ...
            <span className="inline-flex items-center gap-1 rounded-lg bg-white/[0.05] px-2 py-1 text-[10px] text-[#7E7E93]">
        ...
        {STEPS.map((s) => (
          <div key={s.n} className="grid items-center gap-6 md:grid-cols-[56px_1fr_1fr] md:gap-10">
            <div className="hidden md:block">
              <span className="relative z-10 grid h-14 w-14 place-items-center rounded-2xl border border-[#2A2438] bg-[#0C0C13] text-[#7C5CFF]">
            ...
            <div>
              <div className="mb-3 flex items-center gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-xl border border-[#2A2438] bg-[#0C0C13] text-[#7C5CFF] md:hidden">
                  <Icon name={s.icon} size={18} />
                </span>
                <span className="display text-sm text-[#7E7E93]">{s.n}</span>
              </div>
              <h3 className="display text-2xl sm:text-[1.75rem]">{s.title}</h3>
              <p className="mt-2.5 max-w-sm text-[14.5px] leading-relaxed text-[#9B9BAE]">{s.body}</p>
            </div>
