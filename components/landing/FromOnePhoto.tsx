    ...
    <div>
      ...
      <div className="grid items-center gap-7 lg:grid-cols-[minmax(0,290px)_auto_1fr] lg:gap-9">
        ...
        <div className="mx-auto w-full max-w-[240px] lg:max-w-none">
          <div className="relative overflow-hidden rounded-2xl border border-[#E6E2DC] bg-white p-3 shadow-[0_10px_30px_-20px_rgba(20,18,32,0.35)]">
            ...
            />
            <span className="absolute start-5 top-5 inline-flex items-center gap-1.5 rounded-full border border-[#E6E2DC] bg-white/90 px-2.5 py-1 text-[10.5px] font-semibold text-[#6B6678] backdrop-blur">
              <Icon name="image" size={11} />
              {t.beforeTag}
            </span>
          </div>
          <p className="mt-3 text-center text-[12.5px] leading-relaxed text-[#6B6678] lg:text-start">
            {t.beforeNote}
          </p>
        </div>
        ...
        <div className="flex justify-center lg:block">
          <span className="grid h-11 w-11 place-items-center rounded-full border border-[#DED6F2] bg-white text-[#6D4DF6] shadow-[0_6px_18px_-10px_rgba(109,77,246,0.6)]">
        ...
        <div>
          <div className="grid grid-cols-2 gap-2.5 sm:gap-3 lg:grid-cols-4">
            {OUTPUTS.map((file, i) => (
              <figure key={file} className="min-w-0">
                <div className="overflow-hidden rounded-xl border border-[#E6E2DC] bg-[#EDEAE4] sm:rounded-2xl">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                ...
                <figcaption className="mt-2 flex items-baseline gap-1.5">
                  <span className="text-[10px] font-bold tabular-nums text-[#78738A]">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="text-[11.5px] font-semibold leading-tight text-[#3A3548]">
                    {t.jobs[i]}
                  </span>
                </figcaption>
              </figure>
          ...
          <p className="mt-5 flex items-start gap-2 text-[12.5px] leading-relaxed text-[#6B6678]">
            <span className="mt-0.5 shrink-0 text-[#357A38]">
